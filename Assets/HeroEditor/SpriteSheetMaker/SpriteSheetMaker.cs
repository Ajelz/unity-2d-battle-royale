...
namespace Assets.HeroEditor.SpriteSheetMaker
{
    /// <summary>
            ...
            foreach (var upperClip in upperClips)
            {
	            foreach (var lowerClip in lowerClips)
	            {
		            for (var i = 0; i < frameRatio; i++)
		            {
			            ...
			            _screenshotTransparent.Capture(string.Format("{0}/SpriteSheets/{1}-{2}/{3}.png", Application.dataPath.Replace("/Assets", null), upperClip, lowerClip, i));
		            }
	            }
            }
